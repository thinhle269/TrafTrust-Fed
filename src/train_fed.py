...
from .trust import compute_trust_fuzzy
...
def train_federated(
    ...
    method="fedavg", K=10, R=50, E=3, B=32, lr=1e-3, seed=42, input_steps=12,
    ...
    for r in range(1, R+1):
        ...
        # ... sau khi đã có local_weights, sample_counts, val_mae_list, grad_norm_list

# Trust chỉ chạy khi có weights (engine=tf)
    this_round_trust = None
    if (method == "tarfed_fuzzy") and (not use_sklearn):
        this_round_trust = compute_trust_fuzzy(
        local_weights, global_weights, val_mae_list, grad_norm_list
    )
        for k, t in enumerate(this_round_trust):
         trust_rows.append({"round": r, "client": k, "trust": t})

        if use_sklearn:
             new_weights = global_weights  # sklearn: không aggregate bằng weights
        else:
            if method == "fedavg":
        ...
        if r==R and cm is not None:
            import numpy as _np
            _np.savetxt(os.path.join(results_dir, f"confusion_matrix_{method}.csv"), cm, fmt="%d", delimiter=",")
...
def train_federated(
    ...
    for r in range(1, R+1):
        ...
        this_round_trust = None
        if (method == "tarfed_fuzzy") and (not use_sklearn):
            this_round_trust = compute_trust_fuzzy(local_weights, global_weights, val_mae_list, grad_norm_list)
            for k, t in enumerate(this_round_trust):
                trust_rows.append({"round": r, "client": k, "trust": t})
