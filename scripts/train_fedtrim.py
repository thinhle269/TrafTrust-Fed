...
if __name__ == "__main__":
    ...
    ap.add_argument("--trim_rate", type=float, default=0.1)
    ap.add_argument("--engine", choices=["tf","sklearn"], default="tf")
    args = ap.parse_args()

    train_federated(args.data_file, args.coords_file, "results",
        method="fedtrim", K=args.clients, R=args.rounds, E=args.local_epochs, B=args.batch_size,
        lr=args.lr, input_steps=args.input_steps, engine=args.engine, trim_rate=args.trim_rate)
